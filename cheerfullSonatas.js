...
var chords = "zaqxswcdevfrbgtnhymju,ki.lo/;p";
var numberOfKeys = chords.length;
var usedNotes = [];
var newNotes = [];
initMap();
initNotes();
...
function printNotesForRobot(){
	var melody ="{";
	for(i in noteMap){
		if(noteMap[i].length==0) melody+="\"'\",";
		else{
			melody+="\"";
			for(j in noteMap[i]){
				melody+=chords[noteMap[i][j]];
	    	}
			melody+="\",";
	    }
	}
